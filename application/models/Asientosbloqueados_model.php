...
class Asientosbloqueados_model extends CI_Model{
	...
	public function getAsientosBloqueadoSesion($idSesion){
		$asiento = R::getAll("SELECT * FROM asientosbloqueados WHERE id_sesion = $idSesion");
	...
	public function borrarAsientosBloqueados($asiento,$sesion,$usuario) {
		$id = R::getAll( 'SELECT a.id FROM asientosbloqueados a where id_asiento = '.$asiento.' and id_sesion = '.$sesion.' and id_usuario = '.$usuario.' '  );
